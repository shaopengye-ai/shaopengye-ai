...
Review(ThisHotkey) {
    ...
    TagLength := 9
    ; Wiki 链接
    if RegExMatch(Str, "^\[\[.+\]\] #[ik][0-9]/[0-9]{2}-[0-9]{2} [+=-]$") {
        SeparatorPos := InStr(Str, "]] #")
        Item := SubStr(Str, 1, (SeparatorPos + 2) - 1)
        Tag := SubStr(Str, SeparatorPos + 3, TagLength)
    }
    ; Markdown 链接
    else if RegExMatch(Str, "^\[.+\]\(.+\) #[ik][0-9]/[0-9]{2}-[0-9]{2} [+=-]$") {
        SeparatorPos := InStr(Str, ") #")
        Item := SubStr(Str, 1, (SeparatorPos + 1) - 1)
        Tag := SubStr(Str, (SeparatorPos + 2), TagLength)
    }
    ; 非法
    else {
        return
